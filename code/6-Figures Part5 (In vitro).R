...
In.Vitro.Colours = read.csv("Colour_InVitro_Histo.csv")
...
corrplot(as.matrix(cor(P1)),is.corr = F,
...
ggplot(df_vitro, aes(x=Passage_n, y=bc, colour = Exp, group=Exp))+
  geom_point()+
  stat_summary(aes(y = bc), fun.y=mean, colour="red", geom="line")+
  theme(axis.text.x = element_text(angle =90))+
  theme_classic()

## --- Stacked histogram and bubble passaging overtime
...
vitro.SH = Stacked_histo(vitro_clean, angle.x = 90, my_col = vitro.col)
vitro.SH$Stack.plot #6x25 and #10x25

vitro.bbp = Bubble_plot(vitro_clean, Y = "Random", angle.x = 90)
vitro.bbp$BBP
...
1000000/10000
min(MDA.tum.vitro.P10$Pop_P0[MDA.tum.vitro.P10$Pop_P0>0])/10000
max(MDA.tum.vitro.P10$Pop_P0[MDA.tum.vitro.P10$Pop_P0>0])/10000
...
MDA.tum.vitro.P10$Y = vitro_clean$`Population-P0`
bubble.plot.MDA.Tumour.and.P10 = Bubble_plot(MDA.tum.vitro.P10, angle.x = 90, my_col = vitro.col, Y = "input", vitro_clean$`Population-P0`) #6x15
bubble.plot.MDA.Tumour.and.P10$BBP
...
df_Biomass_vitro_vitro = data.frame(matrix(nrow=0, ncol=3))
colnames(df_Biomass_vitro_vitro) = c("names", "biomass", "passage")
length_bc_in_top = vector()
vitro_clean2 = vitro_clean
names(vitro_clean2) = paste0(names(vitro_clean), "_")
for (i in 1:31) {
  ...
  length_bc_in_top = c(length_bc_in_top, length(unique(top10_all_flask)))
  vitro_temp = vitro_clean2[,grep(c, names(vitro_clean2))]
  df_Biomass_vitro_vitro = rbind(df_Biomass_vitro_vitro, data.frame(names = names(vitro_temp),
                                                                    biomass_vitro = colSums(vitro_temp[unique(top10_all_flask),])/10000,
                                                                    passage_dis = c,
                                                                    passage_con = i))
}

df_Biomass_vitro_vitro %>% filter(passage_con %in%c(1,seq(5,15,5)))%>%
  group_by(passage_con) %>%
  dplyr::mutate(median_bio= median(biomass_vitro), sd_bio = sd(biomass_vitro))%>%
  ungroup()%>%
  ggplot(aes(x=factor(passage_con), y=biomass_vitro))+
  geom_bar(stat = "summary", fun = "median")+
  geom_errorbar(aes(ymin=median_bio-sd_bio, ymax=median_bio+sd_bio), width=0.1)+
  geom_quasirandom(size=2, width = 0.03)+
  theme_classic()
...
df_Biomass_vitro_vitro %>% filter(passage_con %in%c(1,seq(5,15,5)))%>%
  group_by(passage_con) %>% dplyr::summarise(Median_bio = median(biomass_vitro))  
...
# Number of barcodes to reach 95% biomass:
length_bc_in_top

head(df_Biomass_vitro_Tum)

ggplot(df_Biomass_vitro_Tum2, aes(x=passage_con, y=biomass))+
  geom_jitter()+
  stat_summary(fun.y=mean, colour="red", geom="line")+
  stat_summary()+
  theme(axis.text.x = element_text(angle =90))+
  theme_classic()
df_Biomass_vitro_Tum2 %>% group_by(passage_con) %>% dplyr::summarise(Mean_bio = mean(biomass), 
                                                                    Median_bio = median(biomass),
                                                                    sd_bio = sd(biomass)) %>%
  pivot_longer(!passage_con, names_to = "stat", values_to = "values") %>%
  ggplot()+
  geom_line(aes(x=passage_con, y=values, colour= stat))+
  geom_line(aes(x=passage_con, y=values, colour= stat))+
  #geom_point(data=data.frame(x=1:31, y=length_bc_in_top), aes(x, y))+
  #scale_y_continuous("% Biomass in tumours", limits = c(0,300), sec.axis = sec_axis(~ . * 1, name = "Number of barcode"))+
  labs(subtitle = "Top 95% barcode invitro contribution in tumours biomass")+
  theme_classic()

df_Biomass_vitro_Tum %>% filter(biomass >90) %>% pull(names) %>% unique()
# tumour 822 and 823 with barcodes 1233 898
stack(vitro_clean[898,])
