...
ggplot()+
  ...
  theme_classic()+
  theme(legend.position = "none")+
  scale_color_manual(values = New.pal(length(unique(df_scatter.MDA$Mouse))) )+
  labs(title = "Scatter plot Lung vs Liver")+
...
scatter.IV = df_scatter %>% filter(Cells == "MDA-231" & MI == "IV") %>%
  ggplot()+
  geom_hline(yintercept=1, linetype = 'dotted')+ geom_vline(xintercept = 1, linetype = 'dotted')+
  geom_point(data= ~subset(.,Lung>0&Liver>0),aes(x=Lung, y=Liver, color=Mouse), alpha=0.9)+
  geom_quasirandom(data= ~subset(.,Lung==0&Liver>0),aes(x=0.0005, y=Liver, color=Mouse ),width=0.1,groupOnX=TRUE,alpha=0.7)+
  geom_quasirandom(data= ~subset(.,Lung>0&Liver==0),aes(x=Lung , y=0.0005, color=Mouse ),width=0.3,groupOnX=F,alpha=0.7)+
  stat_smooth(data= ~subset(.,Lung>0&Liver>0),aes(x=Lung, y=Liver, color=Mouse),method = "lm", se=F)+
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),labels = trans_format("log10", math_format(10^.x)))+
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),labels = trans_format("log10", math_format(10^.x)))+
  #facet_grid(Cells~MI)+
  #scale_x_continuous(limits = c(0,100))+
